...
const osuDifficultyColor = (difficulty) => {
  ...
  const d = clamp(difficulty, 0, 10);

  const segments = [
    { max: 2, start: '#00FF00', end: '#FFFF00' },
    { max: 5, start: '#FFFF00', end: '#FF8000' },
    { max: 7, start: '#FF8000', end: '#FF0000' },
    { max: 9, start: '#FF0000', end: '#8000FF' },
    { max: 10, start: '#8000FF', end: '#8000FF' },
  ];
  ...
  let start, end, startVal;
  for (let i = 0; i < segments.length; i++) {
    if (d <= segments[i].max) {
      start = hexToRgb(segments[i].start);
      end = hexToRgb(segments[i].end);
      startVal = i === 0 ? 0 : segments[i-1].max;
      const t = (d - startVal) / (segments[i].max - startVal);
